...
namespace WebRTC.Server
{
    ...
    public class FFmpegAudioStreamDecoder : IDisposable
    {
        private ILogger logger = SIPSorcery.LogFactory.CreateLogger<FFmpegAudioStreamDecoder>();
        ...
        public unsafe FFmpegAudioStreamDecoder(Stream stream, AVInputFormat* inputFormat = null, bool repeat = false, bool isMicrophone = false)
        {            
            _stream = stream;
            ...
            _readPacketDelegateInstance = new avio_alloc_context_read_packet(ReadPacket);
        }

        private unsafe AVIOContext* InitializeAVIOContext(Stream audioStream)
        {
            const int bufferSize = 4096;

            GCHandle gcHandle = GCHandle.Alloc(audioStream);
            IntPtr opaque = GCHandle.ToIntPtr(gcHandle);

            byte* buffer = (byte*)ffmpeg.av_malloc(bufferSize);

            AVIOContext* avioCtx = ffmpeg.avio_alloc_context(buffer, bufferSize, 0, (void*)opaque, _readPacketDelegateInstance, null, null);
            return avioCtx;
        }
        
        private avio_alloc_context_read_packet _readPacketDelegateInstance;
      
        //note: don't you love p/invokes :)
        private unsafe int ReadPacket(void* opaque, byte* buf, int buf_size)
        {
            GCHandle gcHandle = GCHandle.FromIntPtr((IntPtr)opaque);
            Stream audioStream = (Stream)gcHandle.Target;

            byte[] buffer = new byte[buf_size];
            int bytesRead = audioStream.Read(buffer, 0, buf_size);
        ...
        public unsafe Boolean InitialiseSource(int clockRate)
        {
            if (!_isInitialised)
            {
                ...
                AVIOContext* avioCtx = InitializeAVIOContext(_stream);
                _fmtCtx->pb = avioCtx;

                var pFormatContext = _fmtCtx;
                if (ffmpeg.avformat_open_input(&pFormatContext, null, _inputFormat, null) < 0)
                {
                ...
                ffmpeg.av_dump_format(_fmtCtx, 0, "Memory Stream", 0);
        ...
        private unsafe void RunDecodeLoop()
        {
            ...
            Repeat:
                ...
                {
                    logger.LogDebug($"FFmpeg end of stream.");
